...
class MultiScaleTruchetPattern:
    ...
    def __init__(
        self, how_many_tiles: int, tile_size: int, bg_color: str, fg_color: str
    ):
    ...
    def create_base(self, size: int, bg_color: str) -> Image:
        ...
        tile_img_draw.rectangle(
            [(0, 0), (size + 2 * r, size + 2 * r)], fill=(125, 0, 0, 0)
        )
        ...
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((r, size + r), r), fill=bg_color
        )
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((size + r, r), r), fill=bg_color
        )
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((size + r, size + r), r), fill=bg_color
        )
        return tile_img
    ...
    def create_slash_tile(self, size: int, bg_color: str, fg_color: str) -> Image:
        ...
        tile_img_draw = ImageDraw.Draw(tile_img)
        tile_img_draw.arc(
            self.bb_from_center_and_radius((r, size + r), 2 * r),
            270,
            0,
            fill=fg_color,
            width=int(size * 1 / 3),
        ...
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((r, size / 2 + r), r / 2), fill=fg_color
        )
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((size / 2 + r, r + size), r / 2),
            fill=fg_color,
        )

        # upper arc
        tile_img_draw.arc(
            self.bb_from_center_and_radius((size + r, r), 2 * r),
            90,
            180,
            fill=fg_color,
            width=int(size * 1 / 3),
        ...
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((size / 2 + r, r), r / 2), fill=fg_color
        )
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((size + r, r + size / 2), r / 2),
            fill=fg_color,
        )
        return tile_img
    ...
    def create_minus_tile(self, size: int, bg_color: str, fg_color: str) -> Image:
        ...
        tile_img_draw = ImageDraw.Draw(tile_img)
        tile_img_draw.line(
            [(r, size / 2 + r), (size + r, size / 2 + r)],
            fill=fg_color,
            width=line_height,
        )
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((size / 2 + r, r), small_r), fill=fg_color
        )
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((size / 2 + r, size + r), small_r),
            fill=fg_color,
        )
        # wings
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((size / 3, size / 2 + r), small_r),
            fill=fg_color,
        )
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((size + r, r + size / 2), small_r),
            fill=fg_color,
        )
        return tile_img
    ...
    def create_plus_point_tile(self, size: int, bg_color: str, fg_color: str) -> Image:
        ...
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((r, size / 2 + r), small_r), fill=fg_color
        )
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((size / 2 + r, size + r), small_r),
            fill=fg_color,
        ...
        tile_img_draw.ellipse(
            self.bb_from_center_and_radius((size + r, size / 2 + r), small_r),
            fill=fg_color,
        )
        return tile_img
    ...
    def create_X_point_tile(self, size: int, bg_color: str, fg_color: str) -> Image:
        ...
        tile_img_draw.rectangle(
            [(0, 0), (size + 2 * r, size + 2 * r)], fill=(0, 0, 0, 0)
        )
        tile_img_draw.rectangle([(r, r), (size + r, size + r)], fill=bg_color)
    ...
    def create_plus_tile(self, size: int, bg_color: str, fg_color: str) -> Image:
        ...
        tile_img_draw = ImageDraw.Draw(tile_img)
        tile_img_draw.line(
            [(r, r + size / 2), (r + size - 0.5, r + size / 2)],
            fill=fg_color,
            width=line_height,
        )
        tile_img_draw.line(
            [(r + size / 2, r), (r + size / 2, r + size)],
            fill=fg_color,
            width=line_height,
        )
        # wings
    ...
    def create_fne_tile(self, size: int, bg_color: str, fg_color: str) -> Image:
        ...
        arc_r = 2 / 3 * size - 0.5
        tile_img_draw.arc(
            self.bb_from_center_and_radius((r + size, r), arc_r),
            90,
            182,
            fill=fg_color,
            width=int(size * 1 / 3),
    ...
    def create_base_tile(
        self, size: int, bg_color: str, fg_color: str, kind: str
    ) -> Image:
        match kind:
            case "\\":
    ...
    def paint_a_multiscale_subtile(
        self,
        how_many_subtiles_per_row: int,
        how_many_subtiles_per_column: int,
        subtile_size: int,
        bg_color: str,
        fg_color: str,
    ) -> Image:
        w, h = (
            how_many_subtiles_per_row * subtile_size,
            how_many_subtiles_per_column * subtile_size,
        )
        ...
        for i in range(how_many_subtiles_per_column):
            for j in range(how_many_subtiles_per_row):
                ...
                base_tile = self.create_base_tile(
                    subtile_size, bg_color, fg_color, kind=rd.choice(self.SYMBOLS)
                )
